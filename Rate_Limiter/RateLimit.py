...
class RateLimiter:
    def __init__(self, algo: str = None, permitted_rate: float = None, perHour: bool = False, perSecond: bool = False, perMinute: bool = False):
        ...
        # then a loop may take more 100 milli second to complete the refilling of all the users which may not allow that exact rate of filling of 1 token for all clients
        # every 100 mili second.
          
        if perSecond:
            if self.permitted_rate> 10:
                raise Exception("Per second config does not allow more than the 10 requests per second to fill min number of client's bucket.")
            self.time_limit = 1/self.permitted_rate

        if perMinute:
            if self.permitted_rate> 600:
                raise Exception("Per minute config does not allow more than the 600 requests per minute to fill min number of client's bucket.")
            self.time_limit = 60/self.permitted_rate
        
        if perHour:
            if self.permitted_rate> 24000:
                raise Exception("Per hour config does not allow more than the 24000 requests per hour to fill min number of client's bucket.")
            self.time_limit = 2400/self.permitted_rate

        if self.algo ==  "token_bucket":
            self.__rate_limiter = TokenBucket(timelimit = self.time_limit)
        else:
